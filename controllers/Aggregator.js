	...
	async handleAggregatorLang(lang, slug, request, env, ctx) {
		...
		for (let post of posts) {
			...
			for (let index in postReplacesMap) {
				thisPostHtml = Template.searchAndReplace(thisPostHtml, index, postReplacesMap[index]);
			}
		...
		let header = Template.renderTemplate('header', {
			...
			seo_title: aggregator.seo_title || constants.SITE_NAME + ' - ' + constants.SITE_SLOGAN[lang],
			seo_description:
				aggregator.seo_description || constants.SITE_NAME + ' - ' + constants.SITE_SLOGAN[lang],
